...
async def force_start(c, m):
    """راه‌اندازی مجدد برای رفع خطا - با بررسی عضویت کانال"""
    try:
        user_id = m.chat.id
        
        # اطلاعات پایه
        user_data = get_data("SELECT expir FROM user WHERE id = %s", params=[user_id])
        expir = user_data.get("expir", 0) if user_data else 0
        
        # بررسی ادمین بودن
        is_admin = (user_id == Admin) or (helper_getdata(f"SELECT * FROM adminlist WHERE id = '{user_id}' LIMIT 1") is not None)
        ...
        print(f"Force start successful for {user_id}")
