...
def checker(func):
    ...
    async def wrapper(c, m, *args, **kwargs):
        ...
        # 1. اول عضویت در کانال را چک کن
        is_member = await check_channel_membership(chat_id)
        if not is_member and chat_id != Admin:
            return  # پیام عضویت در channel_required ارسال می‌شود
        
        # 2. مطمئن شو کاربر در دیتابیس وجود دارد
...
def channel_required(func):
    """دکوراتور برای بررسی عضویت در کانال"""
    @wraps(func)
    async def wrapper(c, m, *args, **kwargs):
        user_id = m.chat.id if hasattr(m, "chat") else m.from_user.id
        
        # معافیت برای ادمین
        if user_id == Admin or helper_getdata(f"SELECT * FROM adminlist WHERE id = '{user_id}' LIMIT 1") is not None:
        ...
        if not is_member:
            ...
            keyboard = InlineKeyboardMarkup([
                ...
                [InlineKeyboardButton(text="📚 کانال راهنما", url=f"https://t.me/{Channel_Help}")],
...
📢 **کانال اصلی:** @{Channel_ID}
📚 **کانال راهنما:** @{Channel_Help}

**مراحل:**
1️⃣ روی دکمه‌های بالا کلیک کنید
2️⃣ در کانال‌ها عضو شوید
3️⃣ روی دکمه «بررسی عضویت» کلیک کنید

⚠️ توجه: حتما دکمه «بررسی عضویت» را بزنید تا عضویت شما تایید شود.
"""
...
async def check_channel_membership(user_id):
    """بررسی عضویت کاربر در کانال و گروه پشتیبانی"""
    try:
        # بررسی عضویت در کانال اصلی
        channel_member = await app.get_chat_member(Channel_ID, user_id)
        if channel_member.status in ["left", "kicked"]:
            return False
        
        # بررسی عضویت در کانال راهنما (اختیاری)
        try:
            help_channel_member = await app.get_chat_member(Channel_Help, user_id)
            if help_channel_member.status in ["left", "kicked"]:
                return False
        except:
            pass  # اگر کانال راهنما اجباری نیست
...
async def force_start(c, m):
    """راه‌اندازی مجدد برای رفع خطا - با لاگ‌گیری کامل"""
    try:
        user_id = m.chat.id
        logger.info(f"🔵 START command received from user {user_id}")
        
        # مطمئن شو کاربر وجود دارد
        logger.info(f"🔍 Ensuring user {user_id} exists...")
        if not ensure_user_exists(user_id):
            logger.error(f"❌ Failed to ensure user {user_id} exists")
            await app.send_message(user_id, "**خطا در سیستم، لطفا دوباره تلاش کنید.**")
            return
        
        # اطلاعات پایه
        logger.info(f"📊 Getting user data for {user_id}...")
        user_data = get_data("SELECT expir, step, phone FROM user WHERE id = %s", params=[user_id])
        
        if not user_data:
            logger.warning(f"⚠️ User {user_id} data not found, creating...")
            ensure_user_exists(user_id)
            user_data = {"expir": 0, "step": "none", "phone": None}
        
        expir = user_data.get("expir", 0)
        logger.info(f"📅 User {user_id} expiry: {expir} days")
        
        # بررسی ادمین بودن
        is_admin = False
        try:
            admin_check = helper_getdata(f"SELECT * FROM adminlist WHERE id = '{user_id}' LIMIT 1")
            if admin_check or user_id == Admin:
                is_admin = True
        ...
        # بررسی پوشه سلف
        has_self_folder = os.path.isdir(f"selfs/self-{user_id}")
        logger.info(f"📁 User {user_id} self folder exists: {has_self_folder}")
        ...
        # ایجاد کیبورد
        logger.info(f"⌨️ Creating keyboard for user {user_id}...")
        ...
        # آماده کردن پیام
        user_name = html.escape(m.chat.first_name or "کاربر")
        user_link = f'<a href="tg://user?id={user_id}">{user_name}</a>'
        start_message_template = get_setting("start_message", "**سلام {user_link}، به ربات خرید دستیار تلگرام خوش آمدید!**")
        start_message = start_message_template.format(user_link=user_link)
        
        logger.info(f"📤 Sending welcome message to {user_id}...")
        # ارسال پیام
        await app.send_message(user_id, start_message, reply_markup=keyboard)
        
        # آپدیت step
        logger.info(f"🔄 Updating step for user {user_id} to 'none'")
        update_data("UPDATE user SET step = 'none' WHERE id = %s", params=[user_id])
        
        # پاک کردن temp_Client
        async with lock:
            if user_id in temp_Client:
                logger.info(f"🧹 Cleaning temp_Client for user {user_id}")
                try:
                    if temp_Client[user_id]["client"].is_connected:
        ...
        logger.error(f"❌ ERROR in force_start for user {user_id}: {e}", exc_info=True)
        try:
            await app.send_message(m.chat.id, "**مشکلی پیش آمد. لطفا دوباره /start را بزنید.**")
        except:
            pass
...
لطفا عکس از کارتی که میخواهید با آن خرید انجام دهید ارسال کنید...**""",
        ...
        if is_member:
            await app.edit_message_text(
                ...
                reply_markup=get_main_keyboard(chat_id, expir)
            )
        else:
            keyboard = InlineKeyboardMarkup([
                [InlineKeyboardButton(text="📢 عضویت در کانال", url=f"https://t.me/{Channel_ID}")],
                [InlineKeyboardButton(text="📚 کانال راهنما", url=f"https://t.me/{Channel_Help}")],
                [InlineKeyboardButton(text="🔄 بررسی مجدد", callback_data="check_membership")]
            ])
            ...
            await app.edit_message_text(
                ...
                "1️⃣ روی دکمه‌های بالا کلیک کنید\n"
                "2️⃣ در کانال‌ها عضو شوید\n"
                "3️⃣ سپس روی دکمه «بررسی مجدد» کلیک کنید\n\n"
                "⚠️ توجه: بعد از عضویت، چند ثانیه صبر کنید سپس بررسی کنید.",
